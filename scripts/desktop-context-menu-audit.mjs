...
try {
  ...
  await step('real mail menus cover accounts, folders, labels, conversations, messages, and attachments', async () => {
    ...
    await listThread.waitFor()
    assert.equal(await listThread.getByRole('button').count(), 2)
    const earlierListMessage = listThread.getByRole('button', { name: 'Open message from Earlier Sender' })
    ...
    const realWindowToolbar = realWindow.getByRole('toolbar', { name: 'Conversation actions' })
    await realWindowToolbar.getByRole('button', { name: 'Reply all' }).click()
    const popoutReplyAll = realWindow.getByRole('dialog', { name: 'Reply all' })
    await popoutReplyAll.waitFor()
    assert.match(await popoutReplyAll.locator('.compose-row').filter({ hasText: 'To' }).locator('input').inputValue(), /pilot@aerio\.local/)
    await popoutReplyAll.getByRole('button', { name: 'Close' }).click()
    await realWindowToolbar.getByRole('button', { name: 'Forward' }).waitFor()
    await realWindowToolbar.getByRole('button', { name: 'Remove star' }).click()
    await realWindowToolbar.getByRole('button', { name: 'Add star' }).waitFor()
    await realWindow.getByRole('button', { name: 'Undo', exact: true }).click()
    await realWindowToolbar.getByRole('button', { name: 'Remove star' }).waitFor()
    await realWindowToolbar.getByRole('button', { name: 'Move to trash' }).click()
    await realWindowToolbar.getByRole('button', { name: 'Restore from trash' }).waitFor()
    await realWindow.getByRole('button', { name: 'Undo', exact: true }).click()
    ...
    await menuItem('Forward').click()
    const realCompose = page.getByRole('dialog', { name: 'Forward' })
    ...
    const editDraft = page.getByRole('dialog', { name: 'Edit draft' })
