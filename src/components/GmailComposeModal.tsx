...
export default function GmailComposeModal({ accounts, replyTo, onClose, onSent, onToast }: GmailComposeModalProps) {
  ...
  useEffect(() => {
    const close = (event: KeyboardEvent) => { if (event.key === 'Escape' && status !== 'sending') onClose() }
    ...
    <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}>
      <section className="modal gmail-compose" role="dialog" aria-label="Compose mail message">
        ...
        <div className="compose">
          ...
          <div className="compose-row"><label>Cc</label><input value={cc} onChange={(event) => setCc(event.target.value)} /><button className="text-button" onClick={() => setBccVisible((value) => !value)}>Bcc</button></div>
