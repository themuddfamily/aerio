...
describe('ProductivityStore', () => {
  it('atomically replaces an account snapshot and records sync state', () => {
    ...
    store.replaceAccount('account', 'gmail', data, { contacts: 'sync-token', 'events:remote-calendar': 'event-token' })
    ...
    expect(store.accountData('account')).toEqual(data)
    expect(store.checkpoints('account')).toEqual({ contacts: 'sync-token', 'events:remote-calendar': 'event-token' })
    ...
    expect(store.checkpoints('account')).toEqual({})
